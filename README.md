...
> Cyberboss v0.3.0 — 私人 AI 管家，为小方深度定制

[![License](https://img.shields.io/badge/license-AGPL--3.0--only-blue)](LICENSE)
[![Version](https://img.shields.io/badge/version-0.3.0-green)](https://github.com/Frankoas/customized_cyberboss_xiaofang/releases)
...
- **知识库 Hub 节点**：`[[半导体物理]]` + `[[单片机原理与应用]]` 索引页，按章节/主题组织
- **交叉引用**：所有 quiz 文件底部 `## 🔗 相关笔记`，概念相关互相链接
- **大构思双向链接**：draft ↔ refined 三角闭环 + `[[大构思]]` 总览 hub
- **自动链接引擎**：每日总结 generate 时自动回写 vault 源文件（quiz/flash/draft），补全缺失的 wikilink

### v0.3.0 — 截图分离 + 周/月总结 + 用户反馈

- **Screenshot Tool 拆分**：`cyberboss_timeline_screenshot`（timeline only）和 `cyberboss_summary_screenshot`（HTML summary only）严格分离，参数不重叠
- **周总结**：`templates/weekly-summary.html` — 每周日跟随日总结生成，聚合 7 天数据（时间分配、情绪光谱、闪存亮点、学习进展）
- **月总结**：`templates/monthly-summary.html` — 每月15号跟随日总结生成，含 CSS 热力图、情绪趋势、知识积累、大构思进展
- **实时用户反馈**：`用户反馈/YYYY-MM-DD.md` — LLM 收到 bug/建议时自动写入 Obsidian vault
...
| **v0.3.0** | **2026-06-20** | **截图工具严格分离 + 周/月总结模板和生成 + 实时用户反馈** |
